...
function StartGame()
{
    ...
    //EndOfGame(85); //Debug only
}

function BuildTable()
{
    let pan = document.getElementById("panGame");
    ...
    for (i = 0; i < sRow.length; ++i)
    {
        for (j = 0; j < sColumn.length; ++j)
        {
            ...
            let bwidth = pan.offsetWidth / 12 + 'px';
            let bheight = pan.offsetHeight / 8 + 'px';
...
{
    //Clear load button 
    document.getElementById("btnLoad").value = "";
    
    //Reset scores
    document.getElementById("txtMessageDisplay").value  = "New game";
    ...
    for (i = 0; i < sRow.length; ++i)
    {
        for (j = 0; j < sColumn.length; ++j)
        {
            let b = document.getElementById(sColumn[j] + sRow[i]);
            b.setAttribute("data-fruit", "");
    ...
    for (k = 0; k < 72; ++k)
    {
        if (sc.usedStoneNum < 6)
        {
            ...
            if (iRef1 > -1 && iRef2 > -1)
            {
                ...
                let ThisStone = document.getElementById(sStartingPositions[sc.usedStoneNum]);
...
function NextStep()
{
    ...
    if (sc.usedStoneNum < 71)
    {
        ...
        let bNextStone = document.getElementById("btnNextStone");
    ...
    document.getElementById("txtUsedStoneDisplay").value  = sc.usedStoneNum + 1;
    document.getElementById("txtLeftStoneDisplay").value  = 72 - sc.usedStoneNum - 1;

    sc.finalScore = sc.score + sc.fourWayScore + sc.endScore;

    document.getElementById("txtScoreDisplay").value  = sc.finalScore;
    document.getElementById("txtFourWayDisplay").value  = sc.fourWay;
...
function Usable()
{
    if (sc.usedStoneNum >= 5)
    {
        for (ii = 0; ii < sColumn.length; ++ii)
        {
            ...
            {
                ThisStone = document.getElementById(sColumn[ii] + sRow[jj]);
...
function EndOfGame(piFinalScore)
{
    ...
    document.getElementById("txtMessageDisplay").value  = "Your final score " + piFinalScore;

    app.dialog.alert('No more step', 'Game End');
...
function CheckTop10(iScore, piFinalScore) {
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        {
            document.getElementById("txtMessageDisplay").value  = "Your final score " + piFinalScore + " in Top 10";
            iRef++;
        }
    }
    ...
    if (iRef > 0)
    {
        mainView.router.navigate('/top10form/' + piFinalScore +'/');
    }
}
...
function StoneClick(id) {    
    ...
    let ThisStone = document.getElementById(id.id);
    let sRef;
    ...
    if (ThisStone.getAttribute("data-fruit") == "" && sc.usedStoneNum < 71)
    {
        ...
        if (Landing(ThisStone))
        {
            ...
            document.getElementById("txtMessageDisplay").value  = "Position: " + ThisStone.getAttribute("id") 
        ...
        {
            document.getElementById("txtMessageDisplay").value  = "Invalid stone!";
        }
    }  
}
...
function Landing(NeighborStone)
{
    ...
    for (i = 0; i < 4; ++i)
    {
        ...
        {
            NeighborField = document.getElementById(sNeighborName[i]);
...
function restoreGame(textFromFileLoaded)
{
    ...
    obj[0].forEach(function(item) 
    {
        ...
        ThisStone = document.getElementById(item.name);
    ...
    document.getElementById("txtMessageDisplay").value  = "Game loaded";
    document.getElementById("txtScoreDisplay").value  = sc.score;
    document.getElementById("txtUsedStoneDisplay").value  = sc.usedStoneNum + 1;
    document.getElementById("txtLeftStoneDisplay").value  = 72 - sc.usedStoneNum - 1;
    document.getElementById("txtScoreDisplay").value  = sc.finalScore;
    document.getElementById("txtFourWayDisplay").value  = sc.fourWay;
